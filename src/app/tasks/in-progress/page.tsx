...
import type { Item, Project, PriorityId } from '../../../types';

const PRIORITY_ORDER: Record<PriorityId, number> = { high: 0, medium: 1, low: 2 };
    ...
    <div style={{ fontFamily: fonts.body, fontSize: fontSizes.base, lineHeight: 1.6, maxWidth: 1000, paddingBottom: spacing.xxl, background: colors.bgContent, minHeight: '100%' }}>
      <div style={{ marginBottom: spacing.lg, padding: `${spacing.xxl}px ${spacing.xxl}px 0` }}>
        ...
        <div style={{ fontSize: fontSizes.sm, color: colors.textMuted, marginTop: 2 }}>
          {displayRoots.length} task{displayRoots.length !== 1 ? 's' : ''} in progress
        </div>
      </div>
      ...
      <div style={{ padding: `0 ${spacing.xxl}px` }}>
        {displayRoots.length === 0 ? (
          <p style={{ color: colors.textMuted, fontStyle: 'italic' }}>
            {searchQuery.trim() ? 'No match.' : 'All tasks are complete!'}
          </p>
        ) : (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
            {displayRoots.map(item => renderItem(item, 0))}
          </div>
        )}
      </div>
    </div>
