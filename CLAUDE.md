...
- **Category icons**: `Category.icon` (in `lib/constants.ts`) holds a Phosphor `Icon` *component* (`Target` / `Wrench` / `ChatCircle`), not a string — render with `const I = cat.icon; <I size={14} />`. Cannot be used inside `<option>` (SVG won't render there); category `<select>` dropdowns show label only. Priorities still use unicode glyphs (▲◆▽) as `PriorityLevel.icon` strings.
- **Paperclip color**: the link/notes toggle on rows always uses `colors.textMuted` regardless of attachment count — only the badge counter is colored (`colors.purple`).
